...
V4l2Capture::V4l2Capture(const string& device, int width, int height) : 
                        device(device),
                        xres(width),
                        yres(height)
{
...
const RGBImage& V4l2Capture::frame(int timeout)
{
    for (;;) {
        fd_set fds;
        struct timeval tv;
...
void V4l2Capture::init_device(void)
{
    ...
    if (force_format) {
        ...
        if (fmt.fmt.pix.pixelformat != V4L2_PIX_FMT_YUYV)
            ...
            throw runtime_error("V4l2Capture does not support YUYV format. Support for more format need to be added!");
...
int V4l2Capture::get_frame(void *img_buf, unsigned int buf_len)
{
    this->frame();
    ...
    if (buf_len != rgb_frame.size) {
        return -2;
    }
    memcpy(img_buf, rgb_frame.data, buf_len);
    return 0 ;
}
